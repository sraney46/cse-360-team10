...
public class ViewAddRemoveRoles {
  ...
  */
  ...
  /** The label displaying current user details */
  ...
  // GUI Area 2: This section has a scroll pane setup for each user in the
  // database.
  // users will interact with the menus here to add or delete roles

  /** The scroll pane for the user list */
  protected static ScrollPane scrollPane;

  /** The vertical box container for user rows */
  ...
  */
  ...
  /**********
   * <p>
   * Method: static HBox createUserRow(User user)
   * </p>
   * 
   * <p>
   * Description: Creates and configures a single user row as an HBox for display
   * in the user management list.
   * This method assembles all visual components for one user entry, including:
   * username label, roles label, optional action feedback badge, spacer, action
   * area (with role add/remove ComboBoxes,
   * confirm and cancel buttons), and a menu button for role management actions.
   * </p>
   * 
   * <p>
   * The row is styled with a dark background, rounded corners, and responsive
   * width binding to the parent list container.
   * ComboBoxes are initially hidden and only shown when the user selects "Add
   * Role..." or "Remove Role..." from the menu.
   * A custom ListCell is applied to ensure the prompt text ("Select") reliably
   * reappears after selections and resets.
   * </p>
   * 
   * <p>
   * Event handlers are attached to handle role addition/removal, database
   * updates, UI refresh, and visual feedback
   * via a persistent colored badge (green for add, red for remove).
   * </p>
   * 
   * @param user the User object whose username, roles, and role flags (adminRole,
   *             newRole1, newRole2)
   *             are used to populate and update this row
   * @return the fully configured HBox representing one user row, ready to be
   *         added to a list or container
   */
  ...
  protected static HBox createUserRow(User user) {
    ...
    addRoleItem.setOnAction(_ -> {
      ...
      if (!user.getAdminRole())
        addCombo.getItems().add("Admin");
      if (!user.getNewRole1())
        addCombo.getItems().add("Staff");
      if (!user.getNewRole2())
        addCombo.getItems().add("Student");
    ...
    removeRoleItem.setOnAction(_ -> {
      ...
      if (user.getAdminRole())
        removeCombo.getItems().add("Admin");
      if (user.getNewRole1())
        removeCombo.getItems().add("Staff");
      if (user.getNewRole2())
        removeCombo.getItems().add("Student");
    ...
    confirmBtn.setOnAction(_ -> {
      ...
      if (addCombo.isVisible() && addCombo.getValue() != null) {
        ...
        else if (roleToAdd.equals("Staff"))
          user.setRole1User(true);
        else if (roleToAdd.equals("Student"))
        ...
        else if (roleToRemove.equals("Staff"))
          user.setRole1User(false);
        else if (roleToRemove.equals("Student"))
  ...
  /**********
   * <p>
   * Method: hideActionControls(ComboBox<String> addCombo, ComboBox<String>
   * removeCombo,
   * Button confirmBtn, Button cancelBtn, MenuButton menuButton)
   * </p>
   * 
   * <p>
   * Description: Hides the action-related controls in a user row and restores the
   * visibility
   * of the menu button. This method is called after confirming or canceling a
   * role add/remove operation
   * (or on cancel) to clean up the UI by removing the ComboBoxes, confirm button,
   * and cancel button
   * from view, while making the ellipsis menu button visible again for future
   * actions.
   * </p>
   * 
   * <p>
   * It also resets both ComboBoxes by clearing their selected value
   * (setValue(null)) so that
   * the prompt text ("Select") can reappear correctly the next time they are
   * shown.
   * This helps prevent the known JavaFX behavior where the prompt text fails to
   * display after
   * previous selections.
   * </p>
   * 
   * @param addCombo    the ComboBox used for adding a new role (initially hidden)
   * @param removeCombo the ComboBox used for removing an existing role (initially
   *                    hidden)
   ...
   * @param menuButton  the vertical ellipsis MenuButton that opens the add/remove
   *                    options
   */
   ...
   * and adds it to the `userListBox` VBox. It ensures that any newly added users
   * are immediately
   * visible in the GUI without requiring the page to be reloaded.
   * </p>
   * 
   * <p>
   * Typically called after a user has been added to the database or when roles
   * are updated,
   * so that the displayed list accurately reflects the current state of the
   * system.
   * </p>
   * 
   * <p>
   * This method operates on static fields of the class, including `userListBox`
   * and
