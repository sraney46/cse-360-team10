 ...
 * 
 * <p> Description: The First System Startup Page Model. This class is used to hold the 
 * logic for validating user names and provide useful error messages when incorrect user 
 * names are entered.</p>
 * 
 ...
 * @version 1.01        2026-01-17  Added input validation logic to require first
 * 									character to be alphabetic only. Also added dash (-) and 
 * 									underscore (_) as acceptable symbols. Corresponding 
 * 									error return strings were also updated as well. 
 *  
 */
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 1: 
				...
				// '.', '-', '_' -> State 2
						 ...
						 currentChar == '-' ||                          // Check for - (dash)
						 currentChar == '_' ) {							// Check for _ (underscore)
					nextState = 2;
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += 
					...
					"or symbols period (.), dash (-), or underscore (_)\n";
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
		...
		case 2:
			...
			userNameRecognizerErrorMessage +=
				...
				"\nA UserName character after a symbol must be A-Z, a-z, 0-9.\n";
			return userNameRecognizerErrorMessage;
