...
public class ControllerUserLogin {
  ...
  */
  ...
  protected static void doLogin(Stage ts) {
    ...
    if (!result.isValid()) {
        ViewUserLogin.alertUsernamePasswordError.setContentText(result.getMessage());
    ...
    // Checks to see if a OTP is used and then takes the user to change their
    // password.

    if (password.compareTo(actualPassword) != 0 && password.compareTo(
        theDatabase.getCurrentOTP()) != 0) {
      ViewUserLogin.alertUsernamePasswordError.setContentText(
          "Incorrect username/password. Try again!");
      ViewUserLogin.alertUsernamePasswordError.showAndWait();
      return;
